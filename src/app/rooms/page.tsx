          ...
          {ROOMS.map((room) => (
            <Link key={room.slug} href={`/rooms/${room.slug}`} className="group flex flex-col overflow-hidden bg-white transition-shadow hover:shadow-lg">
              ...
              <div className="flex flex-1 flex-col gap-3 p-6">
                <h2 className="font-display text-heading-lg font-semibold text-deep-ocean">{room.name}</h2>
                <p className="font-body text-body-md text-slate">{room.desc}</p>
                <p className="font-body text-body-sm text-slate/70">{room.amenities}</p>
              </div>
            </Link>
          ))}
        </div>
